...
适用：API v3（accessories.36 起；accessories.37 增加显式兼容声明），Forge 1.20.1 / NeoForge 1.21.1。这是未发布开发功能。原有 schema_version: 1 数据定义继续有效，不需要改 ID。

完整独立的 AI 友好 SDK 见 [交付说明](kubejs-sdk/README.md)。维护契约见 compatibility/cat-accessory-api-v3.json，构建时检查旧入口不被删除。
...
物品与效果注册见 [总说明](cat-accessories.md)。复杂机制的可用示例：[mechanisms.js](examples/cat-accessories/server_scripts/mechanisms.js)，两端共用，复制到 server_scripts 即可；配套注册脚本和三种数据来源任选其一。

## 36 件内置饰品的替换示例

见 [36 件示例包说明](examples/cat-accessories-36/README.md)。沿用现有物品与贴图，7 件事件脚本 + 29 件脚本数据定义调用原生机制；并非把寻路、职业弹幕和渲染全部改成 JavaScript。数据与事件须成套安装，避免原生与脚本重复生效。
...
首批 15 件内置饰品维持无充能消耗；示例风暴猫符配置 100 上限、初始 20，展示耗能治疗波。
...
- 原有 schemaVersion、isEquipped、effectValue、equippedItems 保留；apiVersion() 为 3；supportsApi(1/2/3) 为 true（accessories.37 新增），未来升级须保留旧合同/兼容适配器。
...
单元检查随双端 build 执行。真实 KubeJS/Minecraft 集成测试使用隔离的 build/accessory-gametest/run，不接触游戏实例及用户存档：
