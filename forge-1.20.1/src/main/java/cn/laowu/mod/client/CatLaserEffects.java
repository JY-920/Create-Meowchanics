...
@net.minecraftforge.fml.common.Mod.EventBusSubscriber(modid = LaoWuMod.MOD_ID, value = net.minecraftforge.api.distmarker.Dist.CLIENT)
    ...
    public static void solidBeam(PoseStack pose, VertexConsumer out, float length, int rgb) {
        ...
        for (int[] face : faces) {
            for (int index : face) {
                ...
                out.vertex(pose.last().pose(), v[0], v[1], v[2]).color((rgb >> 16) & 255, (rgb >> 8) & 255, rgb & 255, 255).endVertex();
            }
        }
    }
