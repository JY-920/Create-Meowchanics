...
/** Plays the MCP-authored pipa clip on eight cat bones and two instrument bones. */
public final class CatPipaAnimation {
    public static final float SOURCE_SECONDS = 3.2F;
    ...
    private static final String RESOURCE = "/assets/laowu/cat_animation_clips/cat_pipa_performance.json";
    ...
    public static void apply(float age, ModelPart head, ModelPart body,
                             ...
                             ModelPart tail1, ModelPart tail2, ModelPart pipa, ModelPart plectrum) {
        ...
        applyBone(pipa, 8, from, to, alpha);
        applyBone(plectrum, 9, from, to, alpha);
    }
    ...
    private static Clip load() {
        ...
        try (InputStream stream = CatPipaAnimation.class.getResourceAsStream(RESOURCE)) {
            ...
            if (json.get("format_version").getAsInt() != 1
                ...
                throw new IllegalStateException("Unsupported pipa performance clip");
            }
            ...
            if (sourceFrames.size() != 193 || sourceTimes.size() != sourceFrames.size()) {
            ...
            LOGGER.error("Unable to load cat pipa performance; retaining vanilla cat poses", exception);
            return null;
        }
    }
