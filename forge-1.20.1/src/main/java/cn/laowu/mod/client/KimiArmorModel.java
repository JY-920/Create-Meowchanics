...
/** Animated humanoid armor geometry reconstructed from 基米衣.bbmodel. */
public final class KimiArmorModel extends HumanoidModel<LivingEntity> {
    ...
    public static final ModelLayerLocation LAYER = new ModelLayerLocation(
            new ResourceLocation(LaoWuMod.MOD_ID, "kimi_armor"), "main");
    public static final ModelLayerLocation SLIM_LAYER = new ModelLayerLocation(
            new ResourceLocation(LaoWuMod.MOD_ID, "kimi_armor"), "slim");
    ...
    private static LayerDefinition createLayer(boolean slimArms) {
        ...
        // Exact coordinates from the revised 基米衣.bbmodel. Blockbench's
        // humanoid Y axis is inverted around y=24 when expressed as ModelPart
        // coordinates, hence Java Y = 24 - Blockbench Y.
        root.addOrReplaceChild("head",
                CubeListBuilder.create().texOffs(0, 0)
                        .addBox(-3.85F, -7.85F, -3.85F, 7.7F, 7.7F, 7.7F,
                                new CubeDeformation(0.75F), 0.77F, 0.77F)
                        .texOffs(12, 27)
                        .addBox(-1.15F, -6.55F, -5.15F, 2.3F, 1.5F, 0.7F,
                                new CubeDeformation(0.25F), 0.75F, 0.75F)
                        .texOffs(36, 36)
                        .addBox(2.05F, -10.30F, 0.25F, 1.5F, 1.5F, 2.3F,
                                new CubeDeformation(0.25F), 0.75F, 0.75F)
                        // Keep both ears on the same material island, mirrored
                        // left-to-right. The 0.05 px overlap removes the seam
                        // without changing the supplied UV scale.
                        .texOffs(36, 36).mirror()
                        .addBox(-3.55F, -10.30F, 0.25F, 1.5F, 1.5F, 2.3F,
                                new CubeDeformation(0.25F), 0.75F, 0.75F), PartPose.ZERO);
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer consumer, int packedLight,
        ...
        // vanilla ModelPart cube can only express contiguous box UVs, which is
        // why the two ears previously appeared to have different pixel sizes.
        // Render the authored Blockbench head geometry in the copied humanoid
        // head transform, while all other armour pieces remain vanilla parts.
