...
public final class CatGrenadeBoxItem extends Item {
    ...
    public static int count(ItemStack box) {
        ...
        return tag == null ? 0 : Math.max(0, Math.min(CAPACITY, tag.getInt(COUNT_TAG)));
    }
    ...
    private static int insert(ItemStack box, ItemStack grenades) {
        ...
        box.getOrCreateTag().putInt(COUNT_TAG, count(box) + inserted);
    ...
    private static ItemStack extractStack(ItemStack box) {
        ...
        int extracted = Math.min(stored, LaoWuMod.CAT_GRENADE.get().getMaxStackSize());
    ...
    private static void setCount(ItemStack box, int count) {
        if (count <= 0) {
            CompoundTag tag = box.getTag();
            if (tag != null) tag.remove(COUNT_TAG);
            return;
        }
        box.getOrCreateTag().putInt(COUNT_TAG, Math.min(CAPACITY, count));
    }

    private static NonNullList<ItemStack> contents(ItemStack box) {
        NonNullList<ItemStack> result = NonNullList.create();
        int remaining = count(box);
        int stackLimit = LaoWuMod.CAT_GRENADE.get().getMaxStackSize();
        while (remaining > 0) {
    ...
    public Optional<TooltipComponent> getTooltipImage(ItemStack stack) {
        ...
        int vanillaWeightScale = Math.min(64, (count(stack) + 15) / 16);
        return Optional.of(new BundleTooltip(grenades, vanillaWeightScale));
    }
