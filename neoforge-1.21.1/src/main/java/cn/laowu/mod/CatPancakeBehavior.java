...
public final class CatPancakeBehavior {
    ...
    public static void sanitizeCapturedData(CompoundTag entityData) {
        // NeoForge renamed the serialized persistent-data root in 1.21.
        // Accept ForgeData as well so pancakes carried over from 1.20.1 keep
        // their captured cat data when a world is migrated.
        CompoundTag persistentData = entityData.contains("NeoForgeData")
                ? entityData.getCompound("NeoForgeData")
                : entityData.getCompound("ForgeData");
        if (persistentData.contains(PREVIOUS_NO_AI_TAG)) {
            entityData.putBoolean("NoAI", persistentData.getBoolean(PREVIOUS_NO_AI_TAG));
        }
        persistentData.remove(PREVIOUS_NO_AI_TAG);
        persistentData.remove(ROLLER_CONTRAPTION_TAG);
        persistentData.remove(ANCHOR_X_TAG);
        persistentData.remove(ANCHOR_Z_TAG);
        persistentData.remove(ANCHOR_Y_ROT_TAG);
        persistentData.remove(ANCHOR_X_ROT_TAG);
        entityData.put("NeoForgeData", persistentData);
