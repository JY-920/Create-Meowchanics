...
public final class CatAccessories {
    ...
    private static ItemStack decode(Cat cat, CompoundTag entry) { return ItemStack.parseOptional(cat.registryAccess(), entry); }
    ...
    private static boolean canPickUp(Cat cat, ItemEntity item) {
        return item.getTarget() == null || item.getTarget().equals(cat.getOwnerUUID());
    }
