...
public final class CatAccessoryAuras {
    ...
    private static final net.minecraft.resources.ResourceLocation MOVEMENT_ID=net.minecraft.resources.ResourceLocation.fromNamespaceAndPath("laowu","music_accessory_stride");
    ...
    public static void tick(Cat cat) {
        ...
        if(old!=null && Math.abs(old.amount()-bonus)<1e-8)return;
        if(old!=null)speed.removeModifier(MOVEMENT_ID);
        if(bonus>0)speed.addTransientModifier(new AttributeModifier(MOVEMENT_ID,bonus,AttributeModifier.Operation.ADD_MULTIPLIED_TOTAL));
    }
