...
public final class CatPerformanceOutline {
    ...
    private static final ByteBufferBuilder[] STORAGE=new ByteBufferBuilder[LIMIT];
    ...
    // NeoForge passes the world view separately; its optional PoseStack may be empty.
    // Retain the actual camera matrix while the entity stage is still active.
    private static Matrix4f worldView = new Matrix4f(), worldProjection = new Matrix4f();
    private record Part(MeshData mesh,ResourceLocation texture) {}
    ...
    static void clear(){
        ...
        for(int i=0;i<STORAGE.length;i++)if(STORAGE[i]!=null){STORAGE[i].close();STORAGE[i]=null;}
    }
    private static void clearMeshes(){
        for(Capture capture:CAPTURES.values())for(Part part:capture.parts)part.mesh.close();
        CAPTURES.clear();
    }
    ...
    public static void onRenderStage(RenderLevelStageEvent event){
        if(event.getStage()==RenderLevelStageEvent.Stage.AFTER_SKY
            ...
            accepting=false;
            worldView = new Matrix4f(event.getModelViewMatrix());
    ...
    private static <T extends net.minecraft.world.entity.LivingEntity> void captureBody(
        ...
        if(previous!=null){for(Part part:previous.parts)part.mesh.close();previous.parts.clear();}
        if(STORAGE[slot]==null)STORAGE[slot]=new ByteBufferBuilder(32768);
        BufferBuilder vertices=new BufferBuilder(STORAGE[slot],VertexFormat.Mode.QUADS,DefaultVertexFormat.NEW_ENTITY);
        model.renderToBuffer(poses,vertices,LightTexture.FULL_BRIGHT,OverlayTexture.NO_OVERLAY,-1);
        MeshData data=vertices.build();
    ...
    static void render(RenderLevelStageEvent event){
        ...
        Indicators indicators=indicators(event.getPartialTick().getGameTimeDeltaPartialTick(false));
        ...
        try(State ignored=new State();PerformanceSceneSnapshot.Target target=new PerformanceSceneSnapshot.Target()){
            ...
            if(shader!=null && maskShader!=null && !CAPTURES.isEmpty()){
                ...
                shader.safeGetUniform("Time").set((now%24000+event.getPartialTick().getGameTimeDeltaPartialTick(false))*.05f);
                ...
                b.addVertex(-1,-1,0).setUv(0,0).setColor(-1);b.addVertex(1,-1,0).setUv(1,0).setColor(-1);
                b.addVertex(1,1,0).setUv(1,1).setColor(-1);b.addVertex(-1,1,0).setUv(0,1).setColor(-1);
