...
final class KimiArmorDyeJeiRecipes {
    static List<RecipeHolder<CraftingRecipe>> examples() {
        ...
        for (var armor : List.of(LaoWuMod.CAT_HELMET.get(),LaoWuMod.CAT_CHESTPLATE.get(),
            ...
            for(int example=0;example<5;example++) {
                ...
                var recipe = new ShapedRecipe("kimi_dye",CraftingBookCategory.EQUIPMENT,
                        new ShapedRecipePattern(3,3,ingredients,Optional.empty()),out);
                result.add(new RecipeHolder<CraftingRecipe>(id,recipe));
            }
        }
        for (DyeColor colour : DyeColor.values()) {
            ...
            result.add(new RecipeHolder<CraftingRecipe>(id,
                    new ShapelessRecipe("laser_dye",CraftingBookCategory.EQUIPMENT,out,inputs)));
        }
        return result;
    }
