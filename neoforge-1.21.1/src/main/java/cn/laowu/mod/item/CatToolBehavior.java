...
public final class CatToolBehavior {
    ...
    public static boolean isEmpowermentMarked(ItemStack stack) {
        return isCatHandTool(stack) && ItemCustomData.copy(stack).getBoolean(EMPOWERED_TAG);
    }
    ...
    public static void setEmpowered(ItemStack stack, boolean empowered) {
        ...
        ItemCustomData.update(stack, tag -> {
            if (empowered) {
                tag.putBoolean(EMPOWERED_TAG, true);
            } else {
                tag.remove(EMPOWERED_TAG);
                tag.remove(SWORD_HITS_TAG);
            }
        });
    }
    ...
    public static boolean recordSwordHit(ItemStack stack) {
        ...
        int hits = ItemCustomData.copy(stack).getInt(SWORD_HITS_TAG) + 1;
        boolean armed = hits >= 3;
        ItemCustomData.update(stack, tag -> tag.putInt(SWORD_HITS_TAG, armed ? 0 : hits));
        return armed;
    }
    ...
    /** Synchronized variants used by JEI to demonstrate a 500-point Hiss recharge. */
