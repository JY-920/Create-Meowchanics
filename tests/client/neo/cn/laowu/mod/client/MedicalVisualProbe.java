...
public final class MedicalVisualProbe {
    public static void verify(Minecraft mc) {
        ...
        try (var state = new CatPerformanceOutline.State(); var target = new PerformanceSceneSnapshot.Target()) {
            ...
            for (int mode=0;mode<3;mode++) {
                ...
                model.renderToBuffer(pose,vertices,LightTexture.FULL_BRIGHT,OverlayTexture.NO_OVERLAY,-1);
    ...
    private static Matrix4f capturedWorldView(Matrix4f expected,Matrix4f projection) throws Exception {
        ...
        CatPerformanceOutline.onRenderStage(new net.neoforged.neoforge.client.event.RenderLevelStageEvent(net.neoforged.neoforge.client.event.RenderLevelStageEvent.Stage.AFTER_ENTITIES,null,new PoseStack(),new Matrix4f(expected),new Matrix4f(projection),1,null,null,null));
        ...
        CatPerformanceOutline.onRenderStage(new net.neoforged.neoforge.client.event.RenderLevelStageEvent(net.neoforged.neoforge.client.event.RenderLevelStageEvent.Stage.AFTER_LEVEL,null,poison,new Matrix4f().zero(),new Matrix4f(projection),1,null,null,null));
    ...
    private static void quad(ShaderInstance shader,float x0,float y0,float x1,float y1,float u0,float v0,float u1,float v1) {
        ...
        b.addVertex(x0,y0,0).setUv(u0,v0).setColor(-1);
        b.addVertex(x1,y0,0).setUv(u1,v0).setColor(-1);
        b.addVertex(x1,y1,0).setUv(u1,v1).setColor(-1);
        b.addVertex(x0,y1,0).setUv(u0,v1).setColor(-1);
