    ...
    public static void radiusHostilesAndWalls(GameTestHelper h) {
        ...
        h.assertTrue(CatAgentWatch.marked(near)&&!CatAgentWatch.marked(mid)&&!CatAgentWatch.marked(edge),
                "50 Intelligence has an exact 32-block radius, without a line-of-sight gate: radius="+CatAgentWatch.radius(cat)
                        +", marked="+CatAgentWatch.marked(near)+","+CatAgentWatch.marked(mid)+","+CatAgentWatch.marked(edge)
                        +", nearVisible="+level.getEntitiesOfClass(Mob.class,cat.getBoundingBox().inflate(33)).contains(near)
                        +", nearRegistered="+(level.getEntity(near.getUUID())==near)+", available="+CatAgentWatch.available(cat)
                        +", distance="+Math.sqrt(cat.distanceToSqr(near)));
        ...
        try {
            AgentWatchPacket.encode(packet,buffer);
            h.assertTrue(AgentWatchPacket.decode(buffer).equals(packet)&&!buffer.isReadable(),"Watch S2C packet roundtrip");
