...
public final class CatTraitScriptProbe {
    ...
    private static Cat cat(GameTestHelper h) {
        var cat=EntityType.CAT.create(h.getLevel());cat.setTame(true);cat.setOwnerUUID(OWNER);
    ...
    private static ServerPlayer player(GameTestHelper h,Cat cat) {
        ...
        var player=new ServerPlayer(level.getServer(),level,new GameProfile(UUID.randomUUID(),"trait-probe"));
