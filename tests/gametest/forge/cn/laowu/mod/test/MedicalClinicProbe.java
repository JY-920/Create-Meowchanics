...
public final class MedicalClinicProbe {
    ...
    public static void intelligenceLedgerAndSuitBalance(GameTestHelper h) {
        ...
        try {
            MedicalHealingPacket.encode(packet,buffer);
            h.assertTrue(MedicalHealingPacket.decode(buffer).equals(packet)&&!buffer.isReadable(),
    ...
    public static void selfAndAirbornePilot(GameTestHelper h) {
        ...
        var rider=new ServerPlayer(level.getServer(),level,new GameProfile(UUID.randomUUID(),"med-air-pilot"));
