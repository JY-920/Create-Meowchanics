...
public final class MusicCareerProbe {
    ...
    public static void nineSlotRecordsAndPackets(GameTestHelper h) throws Exception {
        ...
        try {
            MusicSupportPacket.encode(haste,buffer);
            h.assertTrue(MusicSupportPacket.decode(buffer).equals(haste),"Haste packet preserves unbounded percent and server pose");
            MusicRecordPacket.encode(record,buffer);
            h.assertTrue(MusicRecordPacket.decode(buffer).equals(record)&&!buffer.isReadable(),"Record packet preserves UUID, asset, sequence and remaining length");
            var swarm=new CockroachStatePacket(music.getId(),music.getUUID(),10000,2,1199);
            CockroachStatePacket.encode(swarm,buffer);
            h.assertTrue(CockroachStatePacket.decode(buffer).equals(swarm)&&!buffer.isReadable(),"Swarm count/mode/age packet round trip");
