...
public final class PortableWishProbe {
    static ServerPlayer player(GameTestHelper h) {
        ...
        var p=new ServerPlayer(level.getServer(),level,new GameProfile(UUID.randomUUID(),"card-probe"));
    ...
    static ItemStack[] ingredients(int count) {
        ...
        grid[7]=new ItemStack(BuiltInRegistries.ITEM.get(new net.minecraft.resources.ResourceLocation("create","cardboard")),count);
        return grid;
    }
    ...
    public static void manualShiftAndCreateCrafting(GameTestHelper h) {
        ...
        var copied=ItemStack.of(menu.getCarried().save(new CompoundTag()));
        ...
        for(int i=0;i<9;i++){if(input[i].isEmpty())continue;var entry=new CompoundTag();
            entry.putInt("x",i%3);entry.putInt("y",2-i/3);entry.put("item",input[i].save(new CompoundTag()));list.add(entry);}
        data.put("Grid",list);
        bound(h,RecipeGridHandler.tryToApplyRecipe(h.getLevel(),RecipeGridHandler.GroupedItems.read(data)));
        var recipe=(cn.laowu.mod.recipe.WishAdoptionBoxRecipe)h.getLevel().getRecipeManager()
                .byKey(LaoWuMod.id("wish_adoption_box_crafting")).orElseThrow();
        ...
        try {
            serializer.toNetwork(packet,recipe);
            var decoded=serializer.fromNetwork(recipe.getId(),packet);
    ...
    public static void repeatedBreakPlaceAndTrade(GameTestHelper h) {
        ...
        for(boolean locked:new boolean[]{true,false}){
            var tag=box.saveWithoutMetadata();tag.putBoolean("Locked",locked);box.load(tag);
