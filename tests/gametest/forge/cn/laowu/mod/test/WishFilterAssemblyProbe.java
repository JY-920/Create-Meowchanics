...
public final class WishFilterAssemblyProbe {
    private static ServerPlayer player(GameTestHelper h) {
        ...
        var p=new ServerPlayer(level.getServer(),level,new GameProfile(UUID.randomUUID(),"filter-probe"));
    ...
    private static ItemStack roundTrip(ItemStack stack){return ItemStack.of(stack.save(new CompoundTag()));}
    ...
    public static void importResetAndExactPredicate(GameTestHelper h){
        ...
        var stack=p.getMainHandItem();stack.setHoverName(Component.literal("Keep item name"));
    ...
    public static void allIntermediateAssemblyChains(GameTestHelper h){
        ...
        for(String name:names){
            ...
            h.assertTrue(holder instanceof SequencedAssemblyRecipe,"Loaded actual Create sequence "+name);
            var recipe=(SequencedAssemblyRecipe)holder;
            ...
            for(int i=0;i<recipe.getSequence().size();i++){
                ...
                var step=SequencedAssemblyRecipe.getRecipe(h.getLevel(),wrapper,
                        AllRecipeTypes.DEPLOYING.getType(),DeployerApplicationRecipe.class).orElseThrow();
                h.assertTrue(step.matches(wrapper,h.getLevel()),"Actual deployer lookup accepts next stage");
                var outputs=step.rollResults();
