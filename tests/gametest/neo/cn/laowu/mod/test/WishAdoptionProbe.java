...
public final class WishAdoptionProbe {
    ...
    private static CompoundTag save(WishAdoptionBoxBlockEntity box) { return box.saveWithoutMetadata(box.getLevel().registryAccess()); }
    private static void load(WishAdoptionBoxBlockEntity box,CompoundTag tag) { box.loadAdditional(tag,box.getLevel() == null ? net.minecraft.core.RegistryAccess.EMPTY : box.getLevel().registryAccess()); }
    ...
    public static void immediateAndFullOutput(GameTestHelper h) {
        ...
        h.assertTrue(ItemStack.isSameItemSameComponents(box.inventory().getStackInSlot(0),wrong)&&outputs(box)==0,"Nonmatching cat untouched");
        ...
        h.assertTrue(ItemStack.isSameItemSameComponents(box.inventory().getStackInSlot(1),before)&&box.offer().equals(offer(false)),"Full output cannot consume or reroll");
    ...
    public static void refreshAndPersistence(GameTestHelper h) {
        ...
        var update=box.getUpdateTag(h.getLevel().registryAccess());
    ...
    private static ServerPlayer player(GameTestHelper h) {
        ...
        var player=new ServerPlayer(level.getServer(),level,new GameProfile(UUID.randomUUID(),"wish-probe"),net.minecraft.server.level.ClientInformation.createDefault());
