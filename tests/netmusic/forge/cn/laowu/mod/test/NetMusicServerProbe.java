...
public final class NetMusicServerProbe {
    ...
    public static void playlistAndMetadata(GameTestHelper h) throws Exception {
        ...
        cat.setTame(true); cat.setOwnerUUID(UUID.randomUUID());
        ...
        if (expected) {
            var id = new ResourceLocation("netmusic", "music_cd");
    ...
    private static void packet(GameTestHelper h, MusicRecordPacket packet) {
        ...
        try {
            MusicRecordPacket.encode(packet, buf);
            h.assertTrue(packet.equals(MusicRecordPacket.decode(buf)) && !buf.isReadable(),
